...
@jit(nopython=True)
def dd_intersection_over_union(A, B,delta = 0.9) -> float:
    xA = max(A[0], B[0])
    yA = max(A[1], B[1])
    xB = min(A[2], B[2])
    yB = min(A[3], B[3])
    ...
    x_min = max(A[0],B[0])
    y_min = max(A[1],B[1])
    D = ((c_x2-c_x1)**2 + (c_y2-c_y1)**2)/((x_max - x_min)**2 + (y_max - y_min)**2 + 1e-7)
    iou = interArea / float(boxAArea + boxBArea - interArea) - D ** delta
    return iou
...
def find_matching_box(boxes_list, new_box, match_iou):
    ...
    for i in range(len(boxes_list)):
        ...
        iou = dd_intersection_over_union(box[4:], new_box[4:],delta = 0.9)  #diou
        if iou > best_iou:
            best_index = i
            best_iou = iou
